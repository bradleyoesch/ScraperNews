...
try:
  ...
  logging.debug("ValueError while loading items.json: {0}".format(e))
except IOError as e: # if the file can't be found
  logging.debug("IOError while loading items.json: {0}".format(e))
...
try:
  ...
  logging.debug("IOError while loading recents.txt: {0}".format(e))
...
logging.info("Length of recents: {0}".format(recents_len))
...
try:
  ...
  logging.debug("IOError while writing to recents.txt ({0}): {1}".format(item_id, e))
finally:
